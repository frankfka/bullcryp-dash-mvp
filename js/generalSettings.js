// variables
var navigation = document.getElementsByClassName('settingsNav');
...
function Exchange() {
    for (var i = 0; i < account1.exchanges.length; i++){
        display.innerHTML += "<h2>" + account1.exchanges[i].name + "</h2><p>API Key: <input type='text' size='30' value='" + account1.exchanges[i].APIKey + "' name='input'></p><p>API Secret: <input type='text' size='30' value='" + account1.exchanges[i].APISecret + "' name='input'></p>";
    }
}
...
for (var i = 0; i < navigation.length; i++){
    navigation[i].addEventListener('click', function(ev){
        switch (this.innerHTML) {
            case 'General': 
                this.className = "settingsNav current";
                navigation[1].className = "settingsNav";
                navigation[2].className = "settingsNav";
                General();
            break;
            case 'Exchanges': 
                this.className = "settingsNav current";
                navigation[0].className = "settingsNav";
                navigation[2].className = "settingsNav";
                display.innerHTML = "";
                Exchange();
            break;
